...
TEST_ID="validation_error_organization_auth_missing_tenant_id"
...
{"clientId":"org-client-789","clientSecret":"org-secret-abc","authType":"organization"}
JSON

# Given
echo "STEP: Given — prepare organization request payload without tenantId"
echo "PREREQ: using authenticated admin request header for validation path"

# When
echo "STEP: When — POST Outlook auth-url with organization authType and no tenantId"
...
# Then
echo "STEP: Then — assert 400 validation error for missing tenantId"
...
grep -F 'Tenant ID is required for organization-only authentication.' "$BODY_FILE" >/dev/null || { echo "ASSERTION_FAILED: expected missing tenantId validation message"; exit 1; }

echo "CODEVALID_TEST_ASSERTION_OK:validation_error_organization_auth_missing_tenant_id"
