...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="validation_missing_required_prompt_field"
...
printf '{}' > "$REQUEST_BODY_FILE"

echo "STEP: Given — prepare invalid request body without prompt"

echo "STEP: When — POST empty JSON to /api/v1/prompt"
...
echo "CODEVALID_TEST_ASSERTION_OK:validation_missing_required_prompt_field"
