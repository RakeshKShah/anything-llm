...
TEST_ID="token_clear_on_auth_type_change"
...
{"clientId":"seed-common-client","clientSecret":"seed-common-secret","authType":"common"}
JSON
cat > "$REQ2_BODY_FILE" <<'JSON'
{"clientId":"new-client-111","clientSecret":"new-secret-222","tenantId":"tenant-999","authType":"organization"}
JSON

# Given
echo "STEP: Given — establish an initial common auth configuration"
echo "PREREQ: sending first authenticated request to seed existing authType"
...
# When
echo "STEP: When — change authType from common to organization"
echo "REQUEST_HEADERS:"
printf '%s\nContent-Type: application/json\n' "$AUTH_HEADER"
echo "REQUEST_BODY:"
cat "$REQ2_BODY_FILE"
...
# Then
echo "STEP: Then — assert auth-type change succeeds and issues new auth URL"
...
echo "CODEVALID_TEST_ASSERTION_OK:token_clear_on_auth_type_change"
