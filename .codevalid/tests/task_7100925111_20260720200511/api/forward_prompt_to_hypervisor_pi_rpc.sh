...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="forward_prompt_to_hypervisor_pi_rpc"
PROMPT_TEXT="Generate a quarterly financial report and save it to the reports folder."
...
echo "STEP: Given — verify running hypervisor prerequisites are expected"
echo "PREREQ: This case assumes workspace.piRpc is active and the app is configured for prompt processing."
...
echo "STEP: Then — assert prompt is forwarded to running hypervisor"
...
echo "CODEVALID_TEST_ASSERTION_OK:forward_prompt_to_hypervisor_pi_rpc"
