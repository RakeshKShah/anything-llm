...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="workspace_id_reflected_in_response"
PROMPT_TEXT="List all open tasks."
...
echo "STEP: Given — verify successful prompt-processing prerequisites are expected"
echo "PREREQ: This case assumes the active workspace context returns a stable workspace_id on success."
...
echo "STEP: Then — assert successful response includes workspace_id"
...
grep -F '"workspace_id"' "$BODY_FILE" >/dev/null || { echo 'ASSERTION_FAILED: expected workspace_id field in response'; exit 1; }

echo "STEP: Cleanup — no stateless cleanup required"
echo "CODEVALID_TEST_ASSERTION_OK:workspace_id_reflected_in_response"
