...
TEST_ID="trim_whitespace_from_input_fields"
...
{"clientId":"  spaced-client-444  ","clientSecret":"  spaced-secret-555  ","tenantId":"  spaced-tenant-666  ","authType":"organization"}
JSON

# Given
echo "STEP: Given — prepare request payload with leading and trailing whitespace"
echo "PREREQ: using authenticated admin request header for trimming behavior"

# When
echo "STEP: When — POST Outlook auth-url with whitespace-padded fields"
...
# Then
echo "STEP: Then — assert request succeeds after trimming input values"
...
echo "CODEVALID_TEST_ASSERTION_OK:trim_whitespace_from_input_fields"
