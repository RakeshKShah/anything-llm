...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="broadcast_messages_reflect_agent_progress"
PROMPT_TEXT="Find the top 10 customers by revenue and draft a summary."
...
echo "STEP: Given — verify active hypervisor prerequisites are expected"
echo "PREREQ: This case assumes workspace.piRpc is active so the endpoint takes the broadcasting prompt_sent branch."
...
echo "STEP: Then — assert response indicates broadcast-producing happy path"
...
echo "CODEVALID_TEST_ASSERTION_OK:broadcast_messages_reflect_agent_progress"
