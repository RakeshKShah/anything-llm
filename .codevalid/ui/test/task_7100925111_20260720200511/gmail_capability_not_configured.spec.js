...
test("gmail capability shows unconfigured status when deploymentId or apiKey is missing", async ({ page }, testInfo) => {
  const recorder = new ExecutionRecorder({
    testId: "gmail_capability_not_configured",
    testTitle: "Gmail capability shows unconfigured status when deploymentId or apiKey is missing",
  });

  await recorder.step("Set up admin agent skills page and Gmail unconfigured mocks", async () => {
  ...
  await recorder.step("Verify the Gmail capability shows unconfigured warning state", async () => {
  ...
  console.log("CODEVALID_TEST_ASSERTION_OK:gmail_capability_not_configured");
